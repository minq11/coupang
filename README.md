...
- 쿠팡 WING Open API 연동: 내 등록상품 목록을 불러와 순위 조회에 바로 사용
...
5. 순위(광고 포함 전체 순위 + 광고 제외 일반 순위), 상품명, 가격, 별점, 리뷰수가 표시됩니다.
   - 현재 페이지에 없으면 "이 페이지에는 없습니다"로 표시됩니다. 2, 3페이지로 넘어가서 다시 눌러보세요.

## 쿠팡 API 연동 (선택)

API 키를 연결하면 **내 상품** 탭에서 WING에 등록된 상품 목록을 불러오고,
선택한 상품의 ID를 순위 조회 입력창에 자동으로 넣을 수 있습니다.
